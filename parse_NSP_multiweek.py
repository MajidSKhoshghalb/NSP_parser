...
class ModelData(object):
    ...
    def empty_model_data_dict(self, type_file):
        ...
        if type_file[0] == 'S':
            return {
                ...
                'skills': dict(),
                'shiftTypes': dict(),
                'forbiddenShiftTypeSuccessions': dict(),
                'contracts': dict(),
                'nurses': dict()
            }
        if type_file[0] == 'W':
    ...
    def _read_from_file(self, filename, file_type):
        ...
        if file_type is not None and file_type not in valid_file_types:
            raise Exception(
                "Unrecognized file_type {}. Valid file types are {}".format(
            ...
            if filename[-5:] == '.json':
                ...
                raise Exception(
                    "Could not infer type of file {} from its extension!".
                    format(filename))
    ...
    def __init__(self, path, path_folder, inst):
        ...
        for i in range(13):
            if i < 3:
                ...
                globals()[str1] = ModelData(
                    path=self.path,
                    path_folder=self.path_folder,
                    file_type=str('H0' + '-' + path_folder + '-' + str(i)) +
                    '.json')
                self.JSONfileList.append(globals()['H' + str(i) + '-' +
                                                   path_folder + '-' + str(i)])
            elif i < 4:
                global str2
                str2 = 'Sn' + '-' + path_folder
                globals()[str2] = ModelData(
                    path=self.path,
                    path_folder=self.path_folder,
                ...
                globals()[str3] = ModelData(
                    path=self.path,
                    path_folder=self.path_folder,
                    file_type=str('WD' + '-' + path_folder + '-' + str(i - 4))
                    + '.json')
                self.JSONfileList.append(globals()['WD' + '-' + path_folder +
                                                   '-' + str(i - 4)])
        ...
        global W0nbAss
        W0nbAss = []
        findKeys(dataH0[l_h0[2]], 'numberOfAssignments', W0nbAss)

        global W0nbWWeehend
        W0nbWWeehend = []
        findKeys(dataH0[l_h0[2]], 'numberOfWorkingWeekends', W0nbWWeehend)

        global W0LastAssShftType
        W0LastAssShftType = []
        findKeys(dataH0[l_h0[2]], 'lastAssignedShiftType', W0LastAssShftType)

        global W0nbCscDays
        W0nbCscDays = []
        findKeys(dataH0[l_h0[2]], 'numberOfConsecutiveAssignments',
                 W0nbCscDays)
        global W0LastAssShftTypeOff
        W0LastAssShftTypeOff = []
        findKeys(dataH0[l_h0[2]], 'numberOfConsecutiveWorkingDays',
                 W0LastAssShftTypeOff)

        global W0nbCscDaysOff
        W0nbCscDaysOff = []
        findKeys(dataH0[l_h0[2]], 'numberOfConsecutiveDaysOff', W0nbCscDaysOff)
        ...
        global W1nbAss
        W1nbAss = []
        findKeys(dataH1[l_h1[2]], 'numberOfAssignments', W1nbAss)

        global W1nbWWeehend
        W1nbWWeehend = []
        findKeys(dataH1[l_h1[2]], 'numberOfWorkingWeekends', W1nbWWeehend)

        global W1LastAssShftType
        W1LastAssShftType = []
        findKeys(dataH1[l_h1[2]], 'lastAssignedShiftType', W1LastAssShftType)

        global W1nbCscDays
        W1nbCscDays = []
        findKeys(dataH1[l_h1[2]], 'numberOfConsecutiveAssignments',
                 W1nbCscDays)
        global W1LastAssShftTypeOff
        W1LastAssShftTypeOff = []
        findKeys(dataH1[l_h1[2]], 'numberOfConsecutiveWorkingDays',
                 W1LastAssShftTypeOff)

        global W1nbCscDaysOff
        W1nbCscDaysOff = []
        findKeys(dataH1[l_h1[2]], 'numberOfConsecutiveDaysOff', W1nbCscDaysOff)
        ...
        global W2nbAss
        W2nbAss = []
        findKeys(dataH2[l_h2[2]], 'numberOfAssignments', W2nbAss)

        global W2nbWWeehend
        W2nbWWeehend = []
        findKeys(dataH2[l_h2[2]], 'numberOfWorkingWeekends', W2nbWWeehend)

        global W2LastAssShftType
        W2LastAssShftType = []
        findKeys(dataH2[l_h2[2]], 'lastAssignedShiftType', W2LastAssShftType)

        global W2nbCscDays
        W2nbCscDays = []
        findKeys(dataH2[l_h2[2]], 'numberOfConsecutiveAssignments',
                 W2nbCscDays)
        global W2LastAssShftTypeOff
        W2LastAssShftTypeOff = []
        findKeys(dataH2[l_h2[2]], 'numberOfConsecutiveWorkingDays',
                 W2LastAssShftTypeOff)

        global W2nbCscDaysOff
        W2nbCscDaysOff = []
        findKeys(dataH2[l_h2[2]], 'numberOfConsecutiveDaysOff', W2nbCscDaysOff)
        ...
        global ShiftsMinAss
        ShiftsMinAss = []
        findKeys(dataSc[l_s[3]], 'minimumNumberOfConsecutiveAssignments',
                 ShiftsMinAss)

        global ShiftsMaxAss
        ShiftsMaxAss = []
        findKeys(dataSc[l_s[3]], 'maximumNumberOfConsecutiveAssignments',
                 ShiftsMaxAss)

        global forbidShiftPrec
        forbidShiftPrec = []
        findKeys(dataSc[l_s[4]], 'precedingShiftType', forbidShiftPrec)
        ...
        contractMinAss = []
        findKeys(dataSc[l_s[5]], 'minimumNumberOfAssignments', contractMinAss)

        global setShiftsMaxAss
        setShiftsMaxAss = []
        findKeys(dataSc[l_s[5]], 'maximumNumberOfAssignments', setShiftsMaxAss)

        global contractMinCscWdays
        contractMinCscWdays = []
        findKeys(dataSc[l_s[5]], 'minimumNumberOfConsecutiveWorkingDays',
                 contractMinCscWdays)

        global contractMaxCscWdays
        contractMaxCscWdays = []
        findKeys(dataSc[l_s[5]], 'maximumNumberOfConsecutiveWorkingDays',
                 contractMaxCscWdays)
        global contractMinCscWdaysOff
        contractMinCscWdaysOff = []
        findKeys(dataSc[l_s[5]], 'minimumNumberOfConsecutiveDaysOff',
                 contractMinCscWdaysOff)

        global contractMaxCscWdaysOff
        contractMaxCscWdaysOff = []
        findKeys(dataSc[l_s[5]], 'maximumNumberOfConsecutiveDaysOff',
                 contractMaxCscWdaysOff)
...
class ProbDataWeekRequest():
    # Read a NSP instance from an input file

    def __init__(self, path, path_folder, inst, nbweek):
        self.path_folder = path_folder
        self.inst = inst
        self.path = path
        self.nbJSON = 14
        self.JSONfileList = []
        for i in range(self.nbJSON):
            if i < 3:
                global str1
                str1 = 'H' + str(i) + '-' + path_folder + '-' + str(i)
        ...
        for i in range(self.nbJSON):
            if i >= 4:
                # global dataWD0
                dataWD0 = self.get_json_files()[i].data
                global l_wd0
                l_wd0 = [*dataWD0]
                # print("l_wd0l_wd0", l_wd0)
                # print("l_wd0l_wd0", len(l_wd0[1]))
                ...
                for r in range(len(dataWD0[l_wd0[1]])):
                    #print(setShifts)
                    #print(dataWD0[l_wd0[1]][r][l_wd0_req[0]])
...
def nsp_parse(filename, nbweek=None):
    #
    path = os.getcwd()
    ...
    dataweek = ProbDataWeekRequest(path, filename, inst, nbweek)
